...
function createPool(): Pool {
  ...
  return new Pool({
    ...
    port: asInt(process.env.DB_PORT, 5433),
...
const pool = globalWithPool.__metalcardsPgPool ?? createPool();

if (!globalWithPool.__metalcardsPgPool) {
  globalWithPool.__metalcardsPgPool = pool;
}
